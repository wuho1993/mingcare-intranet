  ...
  static async getCareStaff(
    ...
    try {
      ...
      if (filters.search) {
        ...
        query = query.or(`
          name_chinese.ilike.${searchTerm},
          phone.ilike.${searchTerm},
          staff_id.ilike.${searchTerm}
        `)
      }
      ...
      if (filters.has_certificate !== undefined) {
        if (filters.has_certificate) {
          ...
          query = query.and(`
            certificate_1.is.null,
            certificate_2.is.null,
            certificate_3.is.null,
            certificate_4.is.null,
            certificate_5.is.null
          `)
        }
      }
  ...
  static async getCareStaffSuggestions(
    ...
    try {
      ...
      const suggestions: CareStaffSearchSuggestion[] = (data || []).map(item => {
        ...
        if (phoneMatch && staffIdMatch) match_type = 'mixed'
        else if (phoneMatch) match_type = 'phone'
        else if (staffIdMatch) match_type = 'staff_id'
        else match_type = 'name'
