...
export default function EditClientPage({ params }: EditClientPageProps) {
  ...
  const loadCustomer = async () => {
    try {
      const response = await CustomerManagementService.getCustomerById(params.id)
      ...
      if (response.data) {
        ...
        // 設置原始客戶編號
        setOriginalCustomerId(customer.customer_id || '')
        
        // 將資料庫資料轉換為表單資料
        const formDataFromCustomer: CustomerFormData = {
          ...
          district: customer.district as District,
          health_status: customer.health_status as HealthStatus,
          introducer: customer.introducer as Introducer,
          voucher_number: customer.voucher_number || '',
          charity_support: customer.charity_support || false,
          voucher_application_status: customer.voucher_application_status as VoucherApplicationStatus,
          lds_status: customer.lds_status as LdsStatus,
          home_visit_status: customer.home_visit_status as HomeVisitStatus,
          staff_owner: customer.project_manager as StaffOwner,
          copay_level: customer.copay_level as CopayLevel
        }
        
        setFormData(formDataFromCustomer)
      } else {
        setErrors({ general: '找不到客戶資料' })
      }
    } catch (error: any) {
      console.error('Failed to load customer:', error)
      setErrors({ general: '載入客戶資料失敗' })
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await CustomerManagementService.updateCustomer(params.id, updateData as any)
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await CustomerManagementService.deleteCustomer(params.id)
